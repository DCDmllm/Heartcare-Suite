...
class VectorQuantize(Module):
    ...
    def codebook(self):
        ...
        if self.residual_levels > 1:
            return [rearrange(cb, '1 ... -> ...') for cb in codebook]
    ...
    def get_codes_from_indices(self, indices):
        codebooks = self.codebook
        is_multiheaded = codebooks[0].ndim > 2 if isinstance(codebooks, list) else codebooks.ndim > 2

        if not isinstance(codebooks, list):
            if not is_multiheaded:
                codes = codebooks[indices]
            else:
                indices, unpack_one = pack_one(indices, 'b * h')
                indices = rearrange(indices, 'b n h -> b h n')
                indices = repeat(indices, 'b h n -> b h n d', d=codebooks.shape[-1])
                codebooks = repeat(codebooks, 'h n d -> b h n d', b=indices.shape[0])
                codes = codebooks.gather(2, indices)
                codes = rearrange(codes, 'b h n d -> b n (h d)')
                codes = unpack_one(codes, 'b * d')
        else:
            codes = torch.zeros_like(self.codebook[0][0])
            residual = 0
            
            for level, codebook in enumerate(codebooks):
                if not is_multiheaded:
                    quantized = codebook[indices[..., level]]
                else:
                    level_indices, unpack_one = pack_one(indices[..., level], 'b * h')
                    level_indices = rearrange(level_indices, 'b n h -> b h n')
                    level_indices = repeat(level_indices, 'b h n -> b h n d', d=codebook.shape[-1])
                    codebook = repeat(codebook, 'h n d -> b h n d', b=level_indices.shape[0])
                    quantized = codebook.gather(2, level_indices)
                    quantized = rearrange(quantized, 'b h n d -> b n (h d)')
                    quantized = unpack_one(quantized, 'b * d')
    ...
    def get_output_from_indices(self, indices):
        if self.residual_levels == 1:
            return self.project_out(self.get_codes_from_indices(indices))

        codes = torch.zeros_like(self.codebook[0][0])
        for level in range(self.residual_levels):
            codes += self.codebook[level][indices[..., level]]
    ...
    def expire_codes_(self, x):
        ...
        for level in range(self.residual_levels):
            self._codebook[level].expire_codes_(x)
    ...
    def forward(
        ...
        quantize = torch.zeros_like(x)  # RVQ 初始化量化输出
        embed_ind = []  # 存储每层 RVQ 的索引
        residual = x  # 初始 residual

        for level in range(self.residual_levels):
            level_quantize, level_embed_ind, distances = self._codebook(residual, **codebook_forward_kwargs)
            
            embed_ind.append(level_embed_ind)
            quantize = quantize + level_quantize
            residual = residual - level_quantize

        embed_ind = torch.stack(embed_ind, dim=-1)
